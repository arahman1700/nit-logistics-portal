...
import { MOCK_RFIM } from '../../constants';
import { RFIM } from '../../types';
...
export const RfimList: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    return MOCK_RFIM.filter(item => {
      const matchesSearch = 
        item.id.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.mrrvId.toLowerCase().includes(searchQuery.toLowerCase()) ||
              ...
              {filteredData.length > 0 ? (
                filteredData.map((row, idx) => (
                  <tr key={idx} className="nesma-table-row group hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap font-mono text-white">{row.id}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-400 hover:text-nesma-secondary transition-colors cursor-pointer">
                        {row.mrrvId}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">{row.inspectionType}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-nesma-primary/50 flex items-center justify-center text-[10px] text-white font-bold border border-white/10">
                                {row.inspector ? row.inspector.charAt(0) : '?'}
                            </div>
                            <span className="text-gray-400">{row.inspector || 'Unassigned'}</span>
                        </div>
                    </td>
